...
#define SYSFS_uart4_DIR "/dev/tty4"
#define MAX_BUF 64
...
int32_t main (int argc, char *argv[])
{
	...
	sprintf(c_TX_buf, "Teste de envio BBB - Device" );
	...
	while(1)
	{
		int32_t i32_read_bytes = write(uart4, c_TX_buf, MAX_BUF);
        cout << "Bytes escritos - write(): " << i32_read_bytes << " = " << c_TX_buf << endl;  
		usleep(1000000);

        int32_t i32_written_bytes = read(uart4, c_RX_buf, MAX_BUF);
		...
		//int32_t offset = lseek(uart4, 0, SEEK_SET);
		//cout << "Novo offset do RX: " << offset << endl;
		//lseek(uart4, 0, SEEK_SET);
